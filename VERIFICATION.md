...
| Coaches exist and evolve | `cast call 0x94ce4680890ab16b52e3f1a9cdf25c1b01e119b5 "totalMinted()(uint256)" --rpc-url https://evmrpc.0g.ai` — non-zero, and grows as the app is used. Coach `#7` was minted and then evolved twice by a device holding nothing: [v2](https://chainscan.0g.ai/tx/0x987e12489a122403dee9073f7889761fb8c0e5d5a68ba5ded272a5acabb47222) · [v3](https://chainscan.0g.ai/tx/0xa575820404690b5e22e42ca34a2c823fd83101fb17040259768775610695dd3d). `coachOf(7)` reads back version 3. |
| The contracts were deployed here, not migrated in | [CoachAgent](https://chainscan.0g.ai/tx/0xc2cc554f9ce486a11af5cec2d92142111c782ee702cdaaefaada77cc5a017ede) and its [verifier](https://chainscan.0g.ai/tx/0x4e5578dae6a1b407100750936c7b4f1682c28ddda9e87e5afe5dba91537b8c6f), both in block 43752560 |
| Inference is paid for on mainnet | the 0G Compute ledger the coach draws on: [opened and funded](https://chainscan.0g.ai/tx/0x0d926c6db615a9d5554849dd2ee2633269a376931ed64f847a0f1fbf995a79a2). With no ledger no attested provider answers, and the coach refuses rather than downgrading. |
...
| The coach is a registered ERC-8004 Trustless Agent | agent **#3568516** on 0G's mainnet Identity Registry — `cast call 0x8004A169FB4a3325136EB29fA0ceB6D2e539a432 "tokenURI(uint256)(string)" 3568516 --rpc-url https://evmrpc.0g.ai` returns `https://liftwithog.vercel.app/agent-card.json`, and `ownerOf(3568516)` returns `0x495FBf0cc100dE43473003A9385651af5363744D`, the wallet that registered it. [registration tx](https://chainscan.0g.ai/tx/0xb8e3f824a7f2d5ea847ca7ff613f5380ade59805cc2e9fe6a8dedbc8ff673d7a) |
...
```bash
npm --prefix frontend test     # 585 tests — app logic, nutrition, coach memory, sync, offline
npm test                       # 161 tests  — server, storage backends, rate limits, auth, sync
cd contracts && forge test     # 111 tests — 91 unit · 15 fuzz · 5 invariant
```

857 in total. `node scripts/counts.mjs` prints these by running the suites, and
