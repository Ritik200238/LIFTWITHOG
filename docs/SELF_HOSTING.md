...
### Option B — the bundled Caddy (nothing to install)

A fresh VPS runs no reverse proxy, so `docker-compose.https.yml` brings one. Point your
domain's A record at the server, then:

```bash
# .env
RP_ID=gym.example.com
ORIGIN=https://gym.example.com
WEB_PORT=127.0.0.1:8080     # keeps the plain-HTTP port off the public interface
```

```bash
DOMAIN=gym.example.com EMAIL=you@example.com   docker compose -f docker-compose.yml -f docker-compose.https.yml up -d
```

Caddy gets a certificate on first boot and renews it by itself. Ports 80 and 443 have to
be open for that — 80 is how the certificate is issued, not just a redirect.

Already running Caddy yourself? The three lines still work:
...
```bash
tar czf workout-backup-$(date +%F).tar.gz data/
```
...
## 6. Putting the frontend on Vercel

`vercel.json` builds the app and serves it as static files. Two things to know before
you point anyone at that URL.

**It is guest mode only until an API is attached.** Passkeys, sync across devices,
minting or asking a coach all go through the API in `api/`. Without it the app is a
complete local tracker — training, plates, warm-ups, nutrition, and the on-chain reads
the Coaches and Proof screens do straight from the browser — and nothing more.

To attach the API once the VPS from section 3 is up, add its host as a rewrite so the
whole app stays on one origin (passkeys require that — a second hostname breaks them):

```json
{ "source": "/api/:path*", "destination": "https://gym.example.com/api/:path*" }
```

and set `RP_ID` / `ORIGIN` on the VPS to the **Vercel** hostname, since that is the
origin the browser sees.

**Exercise media comes from a CDN there, not from you.** A Vercel deployment has no
`media/` volume, so `/img` and `/gif` are rewritten to the same public dataset the
compose file downloads. Self-hosting through section 1 keeps serving its own copy and
