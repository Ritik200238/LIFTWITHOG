...
/* ============================ starter plan ============================ */
/** Day names for the toast, in the order `Date.getDay()` uses. */
...
export function loadStarterPlan() {
  ...
  /*
   * The plan starts today, not on Monday.
   *
   * It was pinned to Mon/Wed/Fri, so somebody loading it on a Sunday — or any
   * of the four rest days — was immediately told "Rest Day" with nothing to
   * do. That is the first thing a new person sees after asking the app for a
   * plan, and it reads as an app with nothing in it.
   *
   * Every-other-day from today keeps the spacing the split needs and means the
   * answer to "what now" is always a workout.
   */
  const today = new Date().getDay()
  const days = [today, (today + 2) % 7, (today + 4) % 7]

  update(st => {
    st.routines.push(push, pull, legs)
    st.week[days[0]] = push.id
    st.week[days[1]] = pull.id
    st.week[days[2]] = legs.id
  })

  toast(
    t('Starter plan loaded — {0} Push · {1} Pull · {2} Legs',
      t(WEEKDAY[days[0]]), t(WEEKDAY[days[1]]), t(WEEKDAY[days[2]])),
  )
}
...
function BwSheet({ required, onDone, close }) {
  ...
  const bw = lastBW(st)
  /*
   * 70 kg or 155 lb — not 70 either way. The default ignored the unit, so a
   * pounds profile with no history opened the slider at 70 lb: about 32 kg,
   * a child's weight, on the screen that gates every workout.
   */
  const [v, setV] = useState(bw ? bw.w : (unit === 'lb' ? 155 : 70))
  const save = () => {
  ...
  /*
   * Asked first, like every other destructive action in the app. This was a
   * single tap with no confirm and no undo — the only one without a guard, on
   * a list where the rows sit inches apart.
   */
  const delEntry = d => confirmSheet({
    title: t('Delete this weigh-in?'),
    message: t('{0} will be removed from your weight history.', fmtDate(d, true)),
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = 'workout-plan-' + todayISO() + '.json'
  ...
  return <>
    <h3>{t('Plan files')}</h3>
    <div className="muted small" style={{ marginBottom: 16 }}>{t('Send your routines to a friend, put your week on paper, or bring in a plan somebody sent you.')}</div>
    <Button variant="primary" icon="upload" onClick={exportFile} disabled={!hasRoutines}>{t('Export plan file')}</Button>
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own LIFTWITHOG — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
/**
 * Add an exercise to the session in progress: pick it, configure it, and make
 * it the current one.
 *
 * This lived inline on the workout screen's own "Add exercise" button, which is
 * why the finish prompt could only *describe* it — "tap + Add exercise below" —
 * instead of doing it. A button that reads out instructions is a button that
 * does not work, so the flow moved here where both callers can reach it.
 */
...
function WorkoutComplete({ close }) {
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    <div style={{ height: 8 }} />
    <Button icon="plus" onClick={() => { close(); addExerciseFlow() }}>{t('Add another exercise')}</Button>
  </div>
}
export const workoutCompleteSheet = () => ui().openSheet(close => <WorkoutComplete close={close} />, { kind: 'center' })

function FinishSummary({ w, prs, e1prs = [], skipped = [], close }) {
  const st = useStore(s => s.S)
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    {/*
      * An exercise with no checked sets is left out of the saved workout, which
      * is right — it wasn't trained. Saying nothing about it was not: somebody
      * who forgot to tick their last exercise sees it vanish from history with
      * no explanation and assumes the app lost it.
      */}
    {skipped.length > 0 && <div className="small dim" style={{ textAlign: 'left', marginBottom: 12 }}>
      <span className="capitalize">{t('Not saved:')} {skipped.map(id => (EXIDX[id] || {}).n || id).join(', ')}</span>
      {' — '}{t('no sets were checked off.')}
    </div>}
...
function doFinishWorkout() {
  ...
  // Named in the summary rather than dropped in silence.
  const skipped = [...new Set(A.entries.filter(e => !e.sets.some(x => x.done)).map(e => e.id))]
  update(s => {
    w.entries.forEach(e => {
  ...
  ui().openSheet(close => <FinishSummary w={w} prs={prs} e1prs={e1prs} skipped={skipped} close={close} />, { kind: 'center', locked: true })

  /*
   * The flywheel. Every block of training, the coach records what it learned —
   * in the background, with nobody confirming anything and nobody waiting.
   *
   * Deliberately not awaited. The summary is on screen and the person is done;
   * a chain that is slow, or a relayer that is out of funds, must never make a
   * finished workout feel unfinished. It tries again next time.
   */
  void useCoach.getState().maybeEvolve(S())
}

/* ============================ 0G Storage vault ============================ */

/**
 * Backing up to 0G Storage, and getting back the one thing you need to restore.
 *
 * The old flow uploaded and then showed the root hash truncated to ten
 * characters in a toast that vanished after two seconds. Restoring asked you to
 * type the full sixty-six characters into a native `prompt()`. The value you
 * had to type was therefore never displayed anywhere, which made the backup
 * unrestorable by anybody who had not thought to screenshot a toast.
 *
 * So: the hash is shown in full, it can be copied, and — the part that matters
 * — it is remembered on the device, so restoring here is a tap rather than a
 * transcription. It is signed with the device key, which means this works in a
 * home-screen app where `window.ethereum` does not exist.
 */
export function vaultBackupSheet() {
...
function VaultBackup({ close }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const { uploadTo0GVault } = await import('./lib/ogVault.js')
        const { deviceSigner } = await import('./lib/deviceKey.js')
        const { signer } = await deviceSigner()
        const res = await uploadTo0GVault(useStore.getState().S, signer)
        if (gone) return
        setHash(res.rootHash)
        setPhase('done')
        // Remembered, so restoring never means typing this out.
        update(s => {
          s.vaultBackups = [...(s.vaultBackups ?? []), { rootHash: res.rootHash, at: Date.now() }]
        })
      } catch (e) {
        if (gone) return
        setError(e.message || t('The backup could not be uploaded.'))
        setPhase('failed')
      }
    })()
    return () => { gone = true }
  ...
  if (phase === 'working') return <>
    <h3>{t('Backing up to 0G Storage')}</h3>
    <div className="muted small">{t('Encrypting on this device, then uploading. Only your key can open it.')}</div>
  </>

  if (phase === 'failed') return <>
    <h3>{t('Backup failed')}</h3>
  ...
  return <>
    <h3>{t('Backed up')}</h3>
    <div className="muted small">{t('This device remembers it, so restoring here is one tap. Keep a copy to restore on another device.')}</div>
    <div style={{ height: 12 }} />
    <div className="field" style={{ wordBreak: 'break-all', fontSize: 13, lineHeight: 1.5 }}>{hash}</div>
    <div style={{ height: 10 }} />
    <Button icon={copied ? 'check' : 'clipboard'} onClick={async () => {
      try { await navigator.clipboard.writeText(hash); setCopied(true) }
      catch { toast(t('Could not copy — select the text above instead.')) }
    }}>{copied ? t('Copied') : t('Copy the code')}</Button>
    <div style={{ height: 8 }} />
...
/**
 * Restoring — from a backup this device made, or a code from another one.
 *
 * Replacing every workout, weigh-in and setting is the most destructive thing
 * this app can do, and it used to happen the instant a prompt was dismissed,
 * with no confirmation. The JSON import directly above it in settings has
 * always asked first; this now matches it.
 */
export const vaultRestoreSheet = () => ui().openSheet(close => <VaultRestore close={close} />)

function VaultRestore({ close }) {
  const S = useStore(s => s.S)
  const saved = [...(S.vaultBackups ?? [])].reverse()
  const [code, setCode] = useState('')
  const [busy, setBusy] = useState(false)

  const restore = async (rootHash) => {
    setBusy(true)
    try {
      const { downloadFrom0GVault } = await import('./lib/ogVault.js')
      const { deviceSigner } = await import('./lib/deviceKey.js')
      const { signer } = await deviceSigner()
      toast(t('Fetching your backup…'))
      const state = await downloadFrom0GVault(rootHash.trim(), signer)

      const workouts = (state.workouts || []).length
      close()
      confirmSheet({
        title: t('Replace everything on this device?'),
        message: t('That backup holds {0} workouts. Restoring replaces what is here now, and cannot be undone.', workouts),
        confirmText: t('Restore it'),
        danger: true,
        onConfirm: () => {
          useStore.getState().replaceState(state, true)
          toast(t('Restored.'))
        },
      })
    } catch (e) {
      toast(e.message || t('That backup could not be opened.'))
      setBusy(false)
    }
  }

  return <>
    <h3>{t('Restore from 0G Storage')}</h3>
    <div className="muted small">{t('A backup can only be opened by the key that made it.')}</div>

    {saved.length > 0 && <>
      <div style={{ height: 12 }} />
      <div className="muted small">{t('Backups from this device')}</div>
      <div className="list">{saved.map(b => (
        <button type="button" key={b.rootHash} className="item" onClick={() => !busy && restore(b.rootHash)}>
          <span className="lrow-i"><Icon name="download" /></span>
          <div className="grow">
            <div className="tt">{fmtDate(new Date(b.at).toISOString().slice(0, 10), true)}</div>
            <div className="ss" style={{ wordBreak: 'break-all' }}>{b.rootHash.slice(0, 18)}…</div>
          </div>
          <Icon name="chevronRight" className="chev" />
        </button>
      ))}</div>
    ...
    <div style={{ height: 14 }} />
    <div className="muted small">{t('Or paste a code from another device')}</div>
    <div style={{ height: 6 }} />
    <TextField placeholder="0x…" value={code} onChange={e => setCode(e.target.value)} />
    <div style={{ height: 10 }} />
    <Button variant="primary" disabled={busy || !code.trim()} onClick={() => restore(code)}>
      {t('Restore')}
    </Button>
  </>
}
...
/**
 * What you told yourself about this lift, kept for next time.
 *
 * The app remembered every number and nothing anybody actually thought —
 * "elbows in", "the left shoulder complains above 60", "seat on 4". Those are
 * the things that get relearned the hard way every few weeks.
 *
 * One note per exercise rather than per session: it is a standing reminder,
 * not a diary, and a list of thirty old notes is one nobody reads.
 */
export const exerciseNoteSheet = exId => ui().openSheet(close => <ExerciseNote exId={exId} close={close} />)

function ExerciseNote({ exId, close }) {
  const existing = useStore.getState().S.exNotes?.[exId]
  const [text, setText] = useState(existing?.text ?? '')

  const save = () => {
    const clean = text.trim()
    update(s => {
      if (!s.exNotes || typeof s.exNotes !== 'object') s.exNotes = {}
      // An emptied note is removed rather than stored blank, so the button goes
      // back to offering rather than showing an empty line.
      if (clean) s.exNotes[exId] = { text: clean, t: Date.now() }
      else delete s.exNotes[exId]
    })
    close()
  }

  return <>
    <h3>{t('Note for this exercise')}</h3>
    <div className="muted small">{t('Shown every time this lift comes up.')}</div>
    <div style={{ height: 12 }} />
    <TextArea
      autoFocus
      rows={4}
      placeholder={t('Elbows in. Seat on 4. Left shoulder above 60kg…')}
      value={text}
    ...
    {existing && <>
      <div style={{ height: 8 }} />
      <Button className="dim" onClick={() => { setText(''); update(s => { delete s.exNotes?.[exId] }); close() }}>
        {t('Remove the note')}
      </Button>
    </>}
  </>
