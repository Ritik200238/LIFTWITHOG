...
export default function Home() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    /*
     * The dot had no label anywhere. Three states in the same small grey
     * circle — trained, planned, moved — and nothing on the screen said which
     * was which, so it read as decoration.
     */
    const dotMeans = done ? t('trained') : eff ? (ovr ? t('moved here') : t('planned')) : t('rest day')
    strip.push(<div key={i} className={'wday' + (iso === todayISO() ? ' today' : '')} onClick={() => dayOverrideSheet(iso)}
      title={`${d.getDate()} — ${dotMeans}`} aria-label={`${d.getDate()} — ${dotMeans}`}>
  ...
  return <div className="narrow">
    <div className="hdr">
      <div><h1>{user ? t('Hi {0}', user.name) : 'OG_FITNESS'}</h1><div className="sub">{today.toLocaleDateString(dateLocale(), { weekday: 'long', day: 'numeric', month: 'long' })}</div></div>
    ...
    {/*
      * Directly under today's session, which is the one thing everybody looks
      * at. The coach is what makes the training worth keeping — putting it
      * below the charts would make it a feature nobody meets.
      */}
    <CoachCard />
    ...
    {/*
      * Before there is any history, this card said "0 week streak · 0 / 3 this
      * week · 0 workouts total" — three zeros in one line, under two more empty
      * states. A new person's first impression of the app was a report on how
      * little they had done, which is both useless and discouraging.
      *
      * With nothing logged it says what happens next instead. The moment there
      * is one workout it goes back to being the streak card, because then the
      ...
      <div className="row between">
        <div>
          <div className="row" style={{ gap: 7, fontSize: 22, fontWeight: 600, letterSpacing: '-.021em' }}>
            ...
            {S.workouts.length === 0
              ? t('Your streak starts here')
              : t('{0} week streak', streakWeeks(S))}
          </div>
          <div className="muted small" style={{ marginTop: 2 }}>
            {S.workouts.length === 0
              ? t('Finish one session and this fills in.')
