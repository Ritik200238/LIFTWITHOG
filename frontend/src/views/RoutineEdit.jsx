...
export default function RoutineEdit() {
  ...
  // Declared before the early return below, or the hook order changes between
  // renders the moment a routine goes missing.
  const [draftName, setDraftName] = useState(r?.name ?? '')
  useEffect(() => { if (r) setDraftName(r.name) }, [id])
  ...
  /*
   * Going back from a routine that was never touched removes it.
   *
   * Tapping "New" on the Plan screen created and stored a routine before the
   * person had done anything, so backing out left a permanent empty "New
   * routine" in the list with no way to have cancelled.
   */
  const leave = () => {
    const untouched = r.ex.length === 0 && r.name === t('New routine')
    if (untouched) update(s => { s.routines = s.routines.filter(x => x.id !== id) })
    nav('/plan')
  }

  return <div className="narrow">
    <div className="hdr">
      <button className="iconbtn" onClick={leave} aria-label={t('Plan')}><Icon name="chevronLeft" /></button>
      <div style={{ flex: 1, margin: '0 12px' }}>
        {/*
          * Controlled, and only renamed on blur.
          *
          * It was uncontrolled while writing `value.trim() || 'Routine'` on
          * every keystroke — so clearing the field to retype left the store
          * saying "Routine" while the box still looked empty, and the name on
        ...
        <input className="input" aria-label={t('Routine name')} value={draftName} style={{ fontWeight: 600, fontSize: 20, letterSpacing: '-.021em' }}
          ...
          onBlur={() => {
            ...
            setDraftName(name)
            update(s => { s.routines.find(x => x.id === id).name = name })
          }} />
      </div>
      ...
      return <div key={i}>
        ...
        <div className={'item' + (inSS.has(i) ? ' in-ss' : '')} onClick={() => {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 'none', alignItems: 'center' }}>
            {/* `title` is the desktop tooltip; it is also the weakest possible
                accessible name and shows nothing at all on a phone. The label
                is stated, and the link's on/off state with it — the tint was
                the only thing saying whether the superset was on. */}
            {i > 0 && <button className={'iconbtn' + (linkedPrev ? ' on-ss' : '')} title={t('Superset with exercise above')} aria-label={t('Superset with exercise above')} aria-pressed={!!linkedPrev} style={{ width: 32, height: 28, borderRadius: 8, fontSize: 15 }} onClick={ev => { ev.stopPropagation(); toggleLink(i) }}><Icon name="link" /></button>}
