...
export default function Settings() {
  const backupCount = (useStore.getState().S.vaultBackups ?? []).length
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not an OG_FITNESS backup')
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host OG_FITNESS')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
    ...
    {/* ---------- 0G Decentralized AI & Web3 Storage ---------- */}
    <Section title={t('0G Decentralized Vault')}>
      {/*
      ...
      {/*
        ...
        * value the app had never displayed in full anywhere. Both demanded a
        * browser wallet, which does not exist in a home-screen app.
        *
      ...
      <Row icon="download" iconTint="var(--blue)" title={t('Restore a backup')}
        subtitle={backupCount
          ? t(backupCount === 1 ? '{0} backup from this device' : '{0} backups from this device', backupCount)
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install OG_FITNESS as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      OG_FITNESS · Sovereign AI Health Vault<br />
      Built locally · Self-hosted exercise data
    </div>
  </div>
}
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if OG_FITNESS is closed.')}>
