...
export default function Stats() {
  ...
  return <>
    <div className="hdr"><div><h1>{t('Stats')}</h1><div className="sub">{t('Progress & history')}</div></div>
      <Button size="sm" icon="history" onClick={() => nav('/history')}>{t('History')}</Button></div>

    {/*
      * Before there is any training, this screen was four zeroes, an empty
      * twelve-month heatmap and an empty weight chart — a page of blanks that
      * says nothing and offers nothing. One card that explains what will be
      * here, and the button that starts filling it.
      */}
    {S.workouts.length === 0 && (
      <div className="card">
        <h3 style={{ margin: '0 0 4px' }}>{t('Nothing to chart yet')}</h3>
        <div className="muted small">{t('Finish a session and your streak, volume and lifts start appearing here.')}</div>
        <div style={{ height: 12 }} />
        <Button variant="primary" icon="play" onClick={() => nav('/workout')}>{t('Start a workout')}</Button>
      </div>
    )}
